...
if __name__ == '__main__':
    ...
    mujoco_model_path = "/home/ubuntu/isaac/t_s/legged_gym/resources/robots/TOE_dog/xml/scene.xml"
    
    model = mujoco.MjModel.from_xml_path(mujoco_model_path)#载入初始化位置由XML决定
    ...
    for _ in tqdm(range(int(Sim2simCfg.sim_config.sim_duration*10/ Sim2simCfg.sim_config.dt)), desc="Simulating..."):
        ...
        q, dq, quat,omega= get_obs(data)#从mujoco获取仿真数据
        q = q[-12:]
        dq = dq[-12:]
        ...
        if 1:
            ...
            if count_lowlevel % Sim2simCfg.sim_config.decimation == 0:
                ...
                obs[0, 6] = (joy_cmd[0]+command[0])* Sim2simCfg.normalization.obs_scales.lin_vel*0.8
                obs[0, 7] = (joy_cmd[2] +command[1])* Sim2simCfg.normalization.obs_scales.lin_vel*0.6
                obs[0, 8] = (joy_cmd[1] +command[2])* Sim2simCfg.normalization.obs_scales.ang_vel*0.8
                ...
                encoder_input = np.zeros([1, 45*15], dtype=np.float32)
                encoder_output = np.zeros([1, 32], dtype=np.float32) 

                policy_input = np.zeros([1, 45+32], dtype=np.float32) 
                hist_obs_input = np.zeros([1, 45*15], dtype=np.float32)

                # encoder_input[0,0:n_proprio]=obs
                for i in range(15):#缓存历史观测
                    encoder_input[0, i * 45 : (i + 1) * 45] = hist_obs[i]
                ...
                for i in range(32):
                    policy_input[0, i+45] = teacher_latent[0,i]
